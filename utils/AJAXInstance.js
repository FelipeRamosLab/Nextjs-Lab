...
export default class AJAX {
    ...
    async get(params, options) {
        const { isServer, userToken } = Object(options);
        let response;

        try {
            if (!isServer) {
                const token = userToken || await cookieStore.get('token')?.value;
                ...
                response = await axios.get(this.url, { httpsAgent: agent, headers: { token: userToken || '' }, data: params });
            }
